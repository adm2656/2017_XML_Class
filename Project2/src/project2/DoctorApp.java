...
public class DoctorApp {
	...
	String PName, Pssn, Paddress, Pphone;
	...
	private void initialize(String f, String l, String t) {
		...
		btnsave.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (textFieldPName.getText().equals("")){
					...
					p.add(new Patient(PName, Pssn, Paddress, Pphone));
					int index = list_1.getSelectedIndex();
					if(index != -1){
						if (Pssn.equals(checkpatient.get(index).getSsn())){
							try {
								updatepatient(f, l, index, PName, Pssn, Paddress, Pphone);
							} catch (Exception e1) {
								// TODO Auto-generated catch block
								e1.printStackTrace();
							}
							checkpatient.set(index, new Patient(PName, Pssn, Paddress, Pphone));
							...
							try {	
								...
								checkpatient.add(new Patient(PName, Pssn, Paddress, Pphone));
		...
		btnconsult.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				int pa = list_1.getSelectedIndex();
				if (pa == -1){
					JOptionPane.showMessageDialog(null, "You cannot consult without choosing Patient");
				}
				else{
					try{
						String selectssn = checkpatient.get(pa).getSsn();
						consult c = new consult(f, l, selectssn);
						c.con(f, l, selectssn);
					}catch (Exception ex){
		...
		btnconsult.setBounds(405, 229, 130, 29);
		patientspane.add(btnconsult);
				...
				if (qName.equals("Firstname")){
					...
					checklast = str;
						...
						if (qName.equals("PatientName")){
							...
							checkpatient.add(new Patient(checkPname, checkPssn, checkPaddress, checkPphone));
						}
				}
			}
	...
	public void insertPatientdata(String f, String l)throws Exception{
		...
		for (int i = 0; i < firstname.getLength(); i++){
			if (f.equals(firstname.item(i).getNodeValue()) && l.equals(lastname.item(i).getNodeValue())){
				...
				Element PatientAppointment = doc.createElement("Appointments");
				newpatient.appendChild(PatientAppointment);
				}
			}
	...
	public void updatepatient(String f, String l, int c, String PName, String Pssn, String Paddress, String Pphone) throws Exception{
