...
public class Login implements Runnable{
	...
	static String time;
	static String str;
	static String checkfirst, checklast, checktime;
	static Date date;
	static File file;
	
	static ArrayList<Doctor> dc = new ArrayList<Doctor>();
	...
	public void run(){
		try{
			...
			while ((inputline = in.readLine()) != null){
				if (inputline.equals("reservation")){
					out.println("ready");
					String[] appinfo = in.readLine().split(";");
					int judge = 0;
					String pname = appinfo[0];
					String pssn = appinfo[1];
					String dcf = appinfo[2];
					String dcl = appinfo[3];
					String cd = appinfo[4];
					for (int x = 0; x < checkdc.size(); x++){
						if (appinfo[2].equals(checkdc.get(x).getFirst()) && appinfo[3].equals(checkdc.get(x).getLast())){
							judge++;
							System.out.println("We got a reservation from Mr." + pname + ", whose SSN is " + pssn + ".\n" + "Would like to make an appointment with Dr." + dcf + "," + dcl + " at " + cd);
							out.println("success");
							break;
						}
					}
